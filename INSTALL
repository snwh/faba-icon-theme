...
clear
echo '#-----------------------------------------#'
echo '#     Faba Icon Theme Install Script      #'
echo '#-----------------------------------------#'
...
if [ "$UID" -eq "$ROOT_UID" ]; then
	if [ -d /usr/share/icons/Faba ]; then
		...
		case $INPUT in
			[Yy]* ) rm -Rf /usr/share/icons/Faba 2>/dev/null;;
			[Nn]* );;
		    * )
	...
	cp -R ./Faba/ /usr/share/icons/
	chmod -R 755 /usr/share/icons/Faba
	...
	if [ -d $HOME/.local/share/icons/Faba ]; then
		...
		read -p "Found an existing installation. Replace it? (Y)es, (N)o : " INPUT
		case $INPUT in
			[Yy]* ) rm -Rf "$HOME/.local/share/icons/Faba" 2>/dev/null;;
			[Nn]* );;
		    * )
		    clear && echo 'Sorry, try again.'
		    main
	...
	cp -R ./Faba/ $HOME/.local/share/icons/
	echo "Installation complete!"
	set
fi
}
...
function settheme {
echo "Setting Faba as desktop Icon theme..."
...
if [ "$UID" -ne "$ROOT_UID" ]; then
	echo
	echo "Faba Icon Theme will be installed in '$HOME/.local/share/icons/Faba'."
	echo "To make them available to all users, run this script as root."
	continue
else
	echo
	echo "Faba Icon Theme will be installed in '/usr/share/icons'"
